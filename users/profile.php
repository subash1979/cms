...
$branch=$_POST['branch'];
$branchcode=$_POST['branchShortcode'];
$query=mysqli_query($con,"update users set fullName='$fname',contactNo='$contactno',branch='$branch',branchcode='$branchcode' where userEmail='".$_SESSION['login']."'");
if($query)
{
...
<html lang="en">
  <head>
    ...
    <title>Help Desk | User Change Password</title>
...
<option value="<?php echo htmlentities($row['branch']);?>"><?php echo htmlentities($st=$row['branch']);?></option>
...
while ($rw=mysqli_fetch_array($sql)) {
  ...
  {
  ?>
  <option value="<?php echo htmlentities($rw['branchName']);?>"><?php echo htmlentities($rw['branchName']);?></option>
<?php
}}
?>
</select>
...
<div class="col-sm-4">
<select name="branchcode" required="required" class="form-control">
<option value="<?php echo htmlentities($row['branchShortcode']);?>"><?php echo htmlentities($st=$row['branchShortcode']);?></option>
<?php $sql_1=mysqli_query($con,"select branchShortcode from branch ");
while ($rw_1=mysqli_fetch_array($sql_1)) {
  if($rw_1['branchShortcode']==$st_1)
  {
    continue;
  }
  else
  {
  ?>
  <option value="<?php echo htmlentities($rw_1['branchShortcode']);?>"><?php echo htmlentities($rw_1['branchShortcode']);?></option>
<?php
}}
?>
</select>
